...
- Standard Mileage Deduction (calculated): ${o((e.vehicleMilesDriven??0)*a.standardMileageRate)}
...
${e.homeOfficeMethod==="simplified"?`- Simplified Deduction (calculated): ${o(Math.min(e.homeOfficeSqFt??0,a.homeOffice.simplifiedMaxSqFt)*a.homeOffice.simplifiedRatePerSqFt)}`:`- Actual Home Office Expenses: ${o(e.homeOfficeActualExpenses)}`}`:"- No home office"}

## Equipment & Depreciation
${e.hasEquipment?`- Total Equipment Cost This Year: ${o(e.totalEquipmentCost)}
- Section 179 Limit (2026): ${o(a.section179.maxDeduction)}
- Phase-out begins at: ${o(a.section179.phaseOutStart)}
...
${e.isFranchise?`- Royalty Fees Paid: ${o(e.royaltyFees)}
- Advertising Fund Contributions: ${o(e.adFundContributions)}
- Initial Franchise Fee Paid This Year: ${o(e.initialFranchiseFeePaidThisYear)} (amortize over ${v.initialFranchiseFeeAmortizationYears} years per Section 197)`:""}
...
- IRS Standard Mileage Rate: $${a.standardMileageRate}/mile
- Section 179 max deduction: ${o(a.section179.maxDeduction)} (phase-out at ${o(a.section179.phaseOutStart)})
- Bonus depreciation: 100% PERMANENT (OBBBA \u2014 applies to property placed in service after Jan 19, 2025)
- Section 199A QBI deduction: ${(a.qbi.rate*100).toFixed(0)}% (RAISED from 20% by OBBBA \u2014 now permanent)
- QBI thresholds (single): $${a.qbi.thresholds.single.start.toLocaleString()}\u2013$${a.qbi.thresholds.single.end.toLocaleString()}
- QBI thresholds (MFJ): $${a.qbi.thresholds.marriedFilingJointly.start.toLocaleString()}\u2013$${a.qbi.thresholds.marriedFilingJointly.end.toLocaleString()}
- Self-employment tax: 12.4% SS (up to $${a.selfEmploymentTax.socialSecurityWageBase2026.toLocaleString()}) + 2.9% Medicare on all
- Additional Medicare 0.9% above $200K (single) / $250K (MFJ)
- Standard deduction: single $${a.standardDeduction.single.toLocaleString()}, MFJ $${a.standardDeduction.marriedFilingJointly.toLocaleString()}
- Section 197 (franchise fee amortization): 15 years
- Solo 401(k) employee max: $${a.retirement.solo401kEmployee.toLocaleString()} (under 50)
- SEP-IRA: lesser of 25% of compensation or $${a.retirement.sepIra.maxContribution.toLocaleString()}
...
Respond with ONLY the JSON object \u2014 no markdown, no commentary.`}async function V(e){let t={messages:[{role:"user",content:[{text:e}]}],inferenceConfig:{maxTokens:4096,temperature:.1}},n=new T.InvokeModelCommand({modelId:W,contentType:"application/json",accept:"application/json",body:JSON.stringify(t)}),r=await Promise.race([U.send(n),new Promise((d,h)=>setTimeout(()=>h(new Error("BEDROCK_TIMEOUT")),29e3))]),S=JSON.parse(new TextDecoder().decode(r.body)).output.message.content[0].text.replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim();return JSON.parse(S)}async function z(e,t){if(!e.body)return l(400,{error:"Request body is required"});let n;try{n=JSON.parse(e.body)}catch{return l(400,{error:"Invalid JSON in request body"})}if(!n.taxYear||n.taxYear<2e3||n.taxYear>2099)return l(400,{error:"taxYear must be between 2000 and 2099",field:"taxYear"});if(n.totalRevenue===void 0||n.totalRevenue<0)return l(400,{error:"totalRevenue is required and must be non-negative",field:"totalRevenue"});if(n.cogs===void 0||n.cogs<0)return l(400,{error:"cogs is required and must be non-negative",field:"cogs"});if(!n.entityType)return l(400,{error:"entityType is required",field:"entityType"});console.log("Tax calculation request:",JSON.stringify(p(n)));let r=A(),i=new Date().toISOString(),y;try{let d=G(n);y=await V(d)}catch(d){let h=d;return h.message==="BEDROCK_TIMEOUT"?(console.error("Bedrock invocation timed out"),l(504,{error:"Tax analysis timed out. Please try again."})):(console.error("Bedrock invocation failed:",h.message),l(502,{error:"AI model returned an unexpected response. Please try again."}))}let S={userId:t,sk:`TAX#${r}`,sessionId:r,taxYear:n.taxYear,entityType:n.entityType,inputData:p(n),useStandards:n.useStandards,bedrockResponse:y,createdAt:i,status:"complete"};try{await C.send(new m.PutCommand({TableName:F,Item:S}))}catch(d){return console.error("DynamoDB write failed:",d.message),l(500,{error:"Failed to save tax session. Please try again."})}return l(200,{sessionId:r,taxYear:n.taxYear,createdAt:i,result:y})}async function H(e){try{let n=((await C.send(new m.QueryCommand({TableName:F,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",ExpressionAttributeValues:{":uid":e,":prefix":"TAX#"},ScanIndexForward:!1,Limit:100,ProjectionExpression:"sessionId, taxYear, entityType, createdAt, #s",ExpressionAttributeNames:{"#s":"status"}}))).Items??[]).map(r=>({sessionId:r.sessionId,taxYear:r.taxYear,entityType:r.entityType,createdAt:r.createdAt,status:r.status}));return l(200,{sessions:n})}catch(t){return console.error("DynamoDB query failed:",t.message),l(500,{error:"Failed to retrieve tax history."})}}async function Q(e,t){let n=e.pathParameters?.id;if(!n)return l(400,{error:"Session ID is required"});try{let r=await C.send(new m.GetCommand({TableName:F,Key:{userId:t,sk:`TAX#${n}`}}));if(!r.Item)return l(404,{error:"Session not found"});let i=r.Item;return l(200,{sessionId:i.sessionId,taxYear:i.taxYear,entityType:i.entityType,createdAt:i.createdAt,status:i.status,useStandards:i.useStandards,inputData:i.inputData,result:i.bedrockResponse})}catch(r){return console.error("DynamoDB get failed:",r.message),l(500,{error:"Failed to retrieve tax session."})}}var X=async e=>{let t=e.requestContext.authorizer.jwt.claims.sub;switch(e.routeKey){case"POST /tax/calculate":return z(e,t);case"GET /tax/history":return H(t);case"GET /tax/history/{id}":return Q(e,t);default:return l(404,{error:"Route not found"})}};0&&(module.exports={handler});
