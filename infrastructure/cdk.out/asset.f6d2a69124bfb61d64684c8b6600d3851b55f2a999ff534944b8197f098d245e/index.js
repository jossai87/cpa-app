"use strict";var X=Object.create;var E=Object.defineProperty;var Q=Object.getOwnPropertyDescriptor;var Z=Object.getOwnPropertyNames;var ee=Object.getPrototypeOf,te=Object.prototype.hasOwnProperty;var ne=(e,t)=>{for(var o in t)E(e,o,{get:t[o],enumerable:!0})},U=(e,t,o,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let r of Z(t))!te.call(e,r)&&r!==o&&E(e,r,{get:()=>t[r],enumerable:!(n=Q(t,r))||n.enumerable});return e};var Y=(e,t,o)=>(o=e!=null?X(ee(e)):{},U(t||!e||!e.__esModule?E(o,"default",{value:e,enumerable:!0}):o,e)),oe=e=>U(E({},"__esModule",{value:!0}),e);var xe={};ne(xe,{handler:()=>be});module.exports=oe(xe);var g=require("@aws-sdk/client-s3"),j=require("@aws-sdk/s3-request-presigner"),S=require("@aws-sdk/client-bedrock-runtime"),V=require("@aws-sdk/client-dynamodb"),f=require("@aws-sdk/lib-dynamodb");var K=Y(require("crypto")),I=new Uint8Array(256),D=I.length;function R(){return D>I.length-16&&(K.default.randomFillSync(I),D=0),I.slice(D,D+=16)}var c=[];for(let e=0;e<256;++e)c.push((e+256).toString(16).slice(1));function M(e,t=0){return c[e[t+0]]+c[e[t+1]]+c[e[t+2]]+c[e[t+3]]+"-"+c[e[t+4]]+c[e[t+5]]+"-"+c[e[t+6]]+c[e[t+7]]+"-"+c[e[t+8]]+c[e[t+9]]+"-"+c[e[t+10]]+c[e[t+11]]+c[e[t+12]]+c[e[t+13]]+c[e[t+14]]+c[e[t+15]]}var G=Y(require("crypto")),$={randomUUID:G.default.randomUUID};function re(e,t,o){if($.randomUUID&&!t&&!e)return $.randomUUID();e=e||{};let n=e.random||(e.rng||R)();if(n[6]=n[6]&15|64,n[8]=n[8]&63|128,t){o=o||0;for(let r=0;r<16;++r)t[o+r]=n[r];return t}return M(n)}var k=re;var A=new g.S3Client({region:"us-east-1"}),ae=new S.BedrockRuntimeClient({region:"us-east-1"}),se=new V.DynamoDBClient({region:"us-east-1"}),x=f.DynamoDBDocumentClient.from(se),O=process.env.DOCS_BUCKET??"",w=process.env.TABLE_NAME??"",ie=process.env.BEDROCK_MODEL_ID??"us.amazon.nova-2-lite-v1:0",W=process.env.BEDROCK_PRO_MODEL_ID??"us.amazon.nova-pro-v1:0";function _(e,t){return t||new Set(["lease","line-of-credit","bank-statement","profit-loss","general"]).has(e)?W:ie}function a(e,t){return{statusCode:e,headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}}var le=["application/pdf","text/csv","application/vnd.ms-excel","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/msword","image/png","image/jpeg"];function q(e){switch(e.toLowerCase().split(".").pop()??""){case"pdf":return"application/pdf";case"csv":return"text/csv";case"xlsx":return"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";case"xls":return"application/vnd.ms-excel";case"docx":return"application/vnd.openxmlformats-officedocument.wordprocessingml.document";case"doc":return"application/msword";case"png":return"image/png";case"jpg":case"jpeg":return"image/jpeg";default:return"application/octet-stream"}}var ce=["auto","profit-loss","bank-statement","line-of-credit","payroll-summary","royalty-statement","sales-tax-return","fixed-assets","insurance","lease","general"];async function ue(e,t){if(!e.body)return a(400,{error:"Request body is required"});let o;try{o=JSON.parse(e.body)}catch{return a(400,{error:"Invalid JSON in request body"})}if(!o.fileName)return a(400,{error:"fileName is required"});let n=q(o.fileName),r=o.contentType&&o.contentType!==""?o.contentType:n;if(!le.includes(r))return a(400,{error:"Unsupported file type. Allowed: PDF, CSV, XLSX, PNG, JPEG"});let i=ce.includes(o.docType??"")?o.docType:"general",d=`${t}/${i}/${Date.now()}-${k()}-${o.fileName}`;try{let h=new g.PutObjectCommand({Bucket:O,Key:d}),m=await(0,j.getSignedUrl)(A,h,{expiresIn:300});return a(200,{uploadUrl:m,objectKey:d,docType:i,contentType:r,expiresIn:300})}catch(h){return console.error("Failed to create pre-signed URL:",h.message),a(500,{error:"Failed to create upload URL"})}}async function de(e,t){if(!e.body)return a(400,{error:"Request body is required"});let o;try{o=JSON.parse(e.body)}catch{return a(400,{error:"Invalid JSON in request body"})}if(!o.objectKey)return a(400,{error:"objectKey is required"});if(!o.objectKey.startsWith(`${t}/`))return a(403,{error:"Access denied to this document"});let n,r,i=null;try{let s=await A.send(new g.GetObjectCommand({Bucket:O,Key:o.objectKey}));if(r=s.ContentType??"application/octet-stream",r==="application/octet-stream"||r==="binary/octet-stream"){let l=q(o.objectKey);l!=="application/octet-stream"&&(r=l)}if(n=await s.Body.transformToByteArray(),n.byteLength>8*1024*1024)return a(413,{error:`Document is too large for AI extraction (${Math.round(n.byteLength/1024/1024)}MB). Files must be under 8MB. For large reference documents, store them outside this app.`});(r==="text/csv"||r.startsWith("text/"))&&(i=new TextDecoder().decode(n),i.length>5e4&&(i=i.slice(0,5e4)+`

... [truncated]`))}catch(s){return console.error("Failed to read document from S3:",s.message),a(500,{error:"Failed to read uploaded document"})}function d(s){let l=[];if(r==="application/pdf")l.push({document:{format:"pdf",name:"uploaded-document",source:{bytes:Buffer.from(n).toString("base64")}}});else if(r==="image/png"||r==="image/jpeg"||r==="image/jpg"){let y=r==="image/png"?"png":"jpeg";l.push({image:{format:y,source:{bytes:Buffer.from(n).toString("base64")}}})}else r==="application/vnd.ms-excel"||r==="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"?l.push({document:{format:"xlsx",name:"uploaded-document",source:{bytes:Buffer.from(n).toString("base64")}}}):(r==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||r==="application/msword")&&l.push({document:{format:"docx",name:"uploaded-document",source:{bytes:Buffer.from(n).toString("base64")}}});return l.push({text:s}),l}async function h(s,l,y){let u={messages:[{role:"user",content:s}],inferenceConfig:{maxTokens:l,temperature:0}},P=new S.InvokeModelCommand({modelId:y,contentType:"application/json",accept:"application/json",body:JSON.stringify(u)}),L=await Promise.race([ae.send(P),new Promise((Ce,z)=>setTimeout(()=>z(new Error("BEDROCK_TIMEOUT")),25e3))]);return JSON.parse(new TextDecoder().decode(L.body)).output.message.content[0].text}let m=o.docType??"auto",T=null,b=null;if(m==="auto")try{let s=me(i),y=(await h(d(s),4096,_("auto",!0))).replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim(),u=JSON.parse(y);T={classifiedAs:u.classifiedAs,confidence:u.classifyConfidence,rationale:u.classifyRationale,...u.bestGuessLabel&&{bestGuessLabel:u.bestGuessLabel}};let P=["profit-loss","bank-statement","line-of-credit","payroll-summary","royalty-statement","sales-tax-return","fixed-assets","insurance","lease"];u.classifyConfidence==="low"||!P.includes(u.classifiedAs)?m="general":m=u.classifiedAs,b=u.extracted??{}}catch(s){let l=s;if(l.message==="BEDROCK_TIMEOUT")return console.error("Bedrock auto-classify timed out"),a(504,{error:"AI processing timed out. This document may be too long or complex. Try a smaller file or pick the document type manually."});console.warn("Auto classify+extract failed, falling back to general:",l.message),m="general",b=null}if(b===null){let s=pe(m,i),l=d(s);try{let y=m==="bank-statement"||m==="line-of-credit",u=_(m,!1),L=(await h(l,y?4096:2048,u)).replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim();b=JSON.parse(L)}catch(y){let u=y;return u.message==="BEDROCK_TIMEOUT"?(console.error("Bedrock extraction timed out"),a(504,{error:"AI extraction timed out. This document may be too long. Try a smaller file or split it into pages."})):(console.error("Extraction failed:",u.message),a(502,{error:"Failed to extract data from document"}))}}let N=k(),F=new Date().toISOString(),B=o.fileName||o.objectKey.split("/").pop()?.replace(/^\d+-[0-9a-f-]+-/,"")||"document";if(b===null)return a(502,{error:"Extraction returned no data"});let p=b,J=m==="bank-statement"||m==="line-of-credit",v=p.confidence==="low",H=v?{}:J?p.categoryTotals??{}:Object.fromEntries(Object.entries(p).filter(([,s])=>typeof s=="number"&&s>0));try{await x.send(new f.PutCommand({TableName:w,Item:{userId:t,sk:`DOC#${F}#${N}`,docId:N,objectKey:o.objectKey,fileName:B,docType:m,contentType:r,uploadedAt:F,appliedTotals:H,flagged:p.flaggedTransactions??[],bankName:p.bankName??null,periodStart:p.periodStart??null,periodEnd:p.periodEnd??null,confidence:p.confidence??null,notes:p.notes??null,autoClassified:T!==null,autoClassifyResult:T??null}}))}catch(s){console.error("Failed to persist document record:",s.message)}let C=p;return v&&(C=Object.fromEntries(Object.entries(p).filter(([,s])=>typeof s!="number")),C.confidence="low",C.notes=p.notes??"Low confidence \u2014 not auto-applied. Verify manually or pick a specific document type."),a(200,{docId:N,objectKey:o.objectKey,fileName:B,docType:m,contentType:r,uploadedAt:F,extracted:C,autoClassifyResult:T,isLowConfidence:v})}function me(e){return`You are a CPA's document-intake assistant for a Foot Solutions retail franchise in Denton County, Texas.
...
Respond with ONLY the JSON object \u2014 no markdown fences, no explanation.`}function fe(e,t){let o=e==="line-of-credit",n=t?`
...
Respond with ONLY the JSON object.${n}`}async function ye(e){try{let o=((await x.send(new f.QueryCommand({TableName:w,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",ExpressionAttributeValues:{":uid":e,":prefix":"DOC#"},ScanIndexForward:!1,Limit:200}))).Items??[]).map(n=>({docId:n.docId,fileName:n.fileName,docType:n.docType,objectKey:n.objectKey,contentType:n.contentType,uploadedAt:n.uploadedAt,appliedTotals:n.appliedTotals??{},flagged:n.flagged??[],bankName:n.bankName??null,periodStart:n.periodStart??null,periodEnd:n.periodEnd??null,confidence:n.confidence??null,notes:n.notes??null,autoClassified:n.autoClassified??!1,autoClassifyResult:n.autoClassifyResult??null}));return a(200,{documents:o})}catch(t){return console.error("Failed to list documents:",t.message),a(500,{error:"Failed to list documents"})}}async function ge(e,t){let o=e.pathParameters?.id;if(!o)return a(400,{error:"Document id is required"});let n;try{n=(await x.send(new f.QueryCommand({TableName:w,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",FilterExpression:"docId = :docId",ExpressionAttributeValues:{":uid":t,":prefix":"DOC#",":docId":o}}))).Items?.[0]}catch(i){return console.error("Failed to look up document:",i.message),a(500,{error:"Failed to look up document"})}if(!n)return a(404,{error:"Document not found"});let r=n.objectKey;if(!r.startsWith(`${t}/`))return a(403,{error:"Access denied"});try{let i=new g.GetObjectCommand({Bucket:O,Key:r}),d=await(0,j.getSignedUrl)(A,i,{expiresIn:300});return a(200,{downloadUrl:d,fileName:n.fileName,expiresIn:300})}catch(i){return console.error("Failed to create download URL:",i.message),a(500,{error:"Failed to create download URL"})}}async function he(e,t){let o=e.pathParameters?.id;if(!o)return a(400,{error:"Document id is required"});let n;try{n=(await x.send(new f.QueryCommand({TableName:w,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",FilterExpression:"docId = :docId",ExpressionAttributeValues:{":uid":t,":prefix":"DOC#",":docId":o}}))).Items?.[0]}catch(d){return console.error("Failed to look up document for delete:",d.message),a(500,{error:"Failed to delete document"})}if(!n)return a(404,{error:"Document not found"});let r=n.sk,i=n.objectKey;if(!i.startsWith(`${t}/`))return a(403,{error:"Access denied"});try{await A.send(new g.DeleteObjectCommand({Bucket:O,Key:i}))}catch(d){console.error("Failed to delete S3 object:",d.message)}try{await x.send(new f.DeleteCommand({TableName:w,Key:{userId:t,sk:r}}))}catch(d){return console.error("Failed to delete metadata record:",d.message),a(500,{error:"Failed to delete document metadata"})}return a(200,{docId:o,appliedTotals:n.appliedTotals??{},deleted:!0})}var be=async e=>{let t=e.requestContext.authorizer.jwt.claims.sub;switch(e.routeKey){case"POST /documents/upload-url":return ue(e,t);case"POST /documents/extract":return de(e,t);case"POST /documents/bda-job":return a(501,{error:"Not implemented in Phase 1 \u2014 use /documents/extract for CSV"});case"GET /documents":return ye(t);case"GET /documents/{id}/download-url":return ge(e,t);case"DELETE /documents/{id}":return he(e,t);default:return a(404,{error:"Route not found"})}};0&&(module.exports={handler});
