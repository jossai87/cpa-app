"use strict";var M=Object.create;var C=Object.defineProperty;var V=Object.getOwnPropertyDescriptor;var q=Object.getOwnPropertyNames;var J=Object.getPrototypeOf,_=Object.prototype.hasOwnProperty;var H=(e,t)=>{for(var n in t)C(e,n,{get:t[n],enumerable:!0})},$=(e,t,n,o)=>{if(t&&typeof t=="object"||typeof t=="function")for(let r of q(t))!_.call(e,r)&&r!==n&&C(e,r,{get:()=>t[r],enumerable:!(o=V(t,r))||o.enumerable});return e};var j=(e,t,n)=>(n=e!=null?M(J(e)):{},$(t||!e||!e.__esModule?C(n,"default",{value:e,enumerable:!0}):n,e)),z=e=>$(C({},"__esModule",{value:!0}),e);var me={};H(me,{handler:()=>de});module.exports=z(me);var f=require("@aws-sdk/client-s3"),L=require("@aws-sdk/s3-request-presigner"),D=require("@aws-sdk/client-bedrock-runtime"),K=require("@aws-sdk/client-dynamodb"),p=require("@aws-sdk/lib-dynamodb");var Y=j(require("crypto")),E=new Uint8Array(256),S=E.length;function v(){return S>E.length-16&&(Y.default.randomFillSync(E),S=0),E.slice(S,S+=16)}var l=[];for(let e=0;e<256;++e)l.push((e+256).toString(16).slice(1));function U(e,t=0){return l[e[t+0]]+l[e[t+1]]+l[e[t+2]]+l[e[t+3]]+"-"+l[e[t+4]]+l[e[t+5]]+"-"+l[e[t+6]]+l[e[t+7]]+"-"+l[e[t+8]]+l[e[t+9]]+"-"+l[e[t+10]]+l[e[t+11]]+l[e[t+12]]+l[e[t+13]]+l[e[t+14]]+l[e[t+15]]}var B=j(require("crypto")),O={randomUUID:B.default.randomUUID};function X(e,t,n){if(O.randomUUID&&!t&&!e)return O.randomUUID();e=e||{};let o=e.random||(e.rng||v)();if(o[6]=o[6]&15|64,o[8]=o[8]&63|128,t){n=n||0;for(let r=0;r<16;++r)t[n+r]=o[r];return t}return U(o)}var k=X;var A=new f.S3Client({region:"us-east-1"}),Q=new D.BedrockRuntimeClient({region:"us-east-1"}),Z=new K.DynamoDBClient({region:"us-east-1"}),w=p.DynamoDBDocumentClient.from(Z),F=process.env.DOCS_BUCKET??"",T=process.env.TABLE_NAME??"",ee=process.env.BEDROCK_MODEL_ID??"us.amazon.nova-2-lite-v1:0";function a(e,t){return{statusCode:e,headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}}var te=["application/pdf","text/csv","application/vnd.ms-excel","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/msword","image/png","image/jpeg"];function G(e){switch(e.toLowerCase().split(".").pop()??""){case"pdf":return"application/pdf";case"csv":return"text/csv";case"xlsx":return"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";case"xls":return"application/vnd.ms-excel";case"docx":return"application/vnd.openxmlformats-officedocument.wordprocessingml.document";case"doc":return"application/msword";case"png":return"image/png";case"jpg":case"jpeg":return"image/jpeg";default:return"application/octet-stream"}}var ne=["auto","profit-loss","bank-statement","line-of-credit","payroll-summary","royalty-statement","sales-tax-return","fixed-assets","insurance","lease","general"];async function oe(e,t){if(!e.body)return a(400,{error:"Request body is required"});let n;try{n=JSON.parse(e.body)}catch{return a(400,{error:"Invalid JSON in request body"})}if(!n.fileName)return a(400,{error:"fileName is required"});let o=G(n.fileName),r=n.contentType&&n.contentType!==""?n.contentType:o;if(!te.includes(r))return a(400,{error:"Unsupported file type. Allowed: PDF, CSV, XLSX, PNG, JPEG"});let s=ne.includes(n.docType??"")?n.docType:"general",c=`${t}/${s}/${Date.now()}-${k()}-${n.fileName}`;try{let h=new f.PutObjectCommand({Bucket:F,Key:c}),d=await(0,L.getSignedUrl)(A,h,{expiresIn:300});return a(200,{uploadUrl:d,objectKey:c,docType:s,contentType:r,expiresIn:300})}catch(h){return console.error("Failed to create pre-signed URL:",h.message),a(500,{error:"Failed to create upload URL"})}}async function re(e,t){if(!e.body)return a(400,{error:"Request body is required"});let n;try{n=JSON.parse(e.body)}catch{return a(400,{error:"Invalid JSON in request body"})}if(!n.objectKey)return a(400,{error:"objectKey is required"});if(!n.objectKey.startsWith(`${t}/`))return a(403,{error:"Access denied to this document"});let o,r,s=null;try{let i=await A.send(new f.GetObjectCommand({Bucket:F,Key:n.objectKey}));if(r=i.ContentType??"application/octet-stream",r==="application/octet-stream"||r==="binary/octet-stream"){let u=G(n.objectKey);u!=="application/octet-stream"&&(r=u)}o=await i.Body.transformToByteArray(),(r==="text/csv"||r.startsWith("text/"))&&(s=new TextDecoder().decode(o),s.length>5e4&&(s=s.slice(0,5e4)+`

... [truncated]`))}catch(i){return console.error("Failed to read document from S3:",i.message),a(500,{error:"Failed to read uploaded document"})}function c(i){let u=[];if(r==="application/pdf")u.push({document:{format:"pdf",name:"uploaded-document",source:{bytes:Buffer.from(o).toString("base64")}}});else if(r==="image/png"||r==="image/jpeg"||r==="image/jpg"){let g=r==="image/png"?"png":"jpeg";u.push({image:{format:g,source:{bytes:Buffer.from(o).toString("base64")}}})}else r==="application/vnd.ms-excel"||r==="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"?u.push({document:{format:"xlsx",name:"uploaded-document",source:{bytes:Buffer.from(o).toString("base64")}}}):(r==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"||r==="application/msword")&&u.push({document:{format:"docx",name:"uploaded-document",source:{bytes:Buffer.from(o).toString("base64")}}});return u.push({text:i}),u}async function h(i,u){let g={messages:[{role:"user",content:i}],inferenceConfig:{maxTokens:u,temperature:0}},m=new D.InvokeModelCommand({modelId:ee,contentType:"application/json",accept:"application/json",body:JSON.stringify(g)}),x=await Q.send(m);return JSON.parse(new TextDecoder().decode(x.body)).output.message.content[0].text}let d=n.docType??"auto",P=null,b=null;if(d==="auto")try{let i=ae(s),g=(await h(c(i),4096)).replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim(),m=JSON.parse(g);P={classifiedAs:m.classifiedAs,confidence:m.classifyConfidence,rationale:m.classifyRationale,...m.bestGuessLabel&&{bestGuessLabel:m.bestGuessLabel}};let x=["profit-loss","bank-statement","line-of-credit","payroll-summary","royalty-statement","sales-tax-return","fixed-assets","insurance","lease"];m.classifyConfidence==="low"||!x.includes(m.classifiedAs)?d="general":d=m.classifiedAs,b=m.extracted??{}}catch(i){console.warn("Auto classify+extract failed, falling back to general:",i.message),d="general",b=null}if(b===null){let i=se(d,s),u=c(i);try{let x=(await h(u,d==="bank-statement"||d==="line-of-credit"?4096:2048)).replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim();b=JSON.parse(x)}catch(g){return console.error("Extraction failed:",g.message),a(502,{error:"Failed to extract data from document"})}}let N=k(),I=new Date().toISOString(),R=n.fileName||n.objectKey.split("/").pop()?.replace(/^\d+-[0-9a-f-]+-/,"")||"document",y=b,W=d==="bank-statement"||d==="line-of-credit"?y.categoryTotals??{}:Object.fromEntries(Object.entries(y).filter(([,i])=>typeof i=="number"&&i>0));try{await w.send(new p.PutCommand({TableName:T,Item:{userId:t,sk:`DOC#${I}#${N}`,docId:N,objectKey:n.objectKey,fileName:R,docType:d,contentType:r,uploadedAt:I,appliedTotals:W,flagged:y.flaggedTransactions??[],bankName:y.bankName??null,periodStart:y.periodStart??null,periodEnd:y.periodEnd??null,confidence:y.confidence??null,notes:y.notes??null,autoClassified:P!==null,autoClassifyResult:P??null}}))}catch(i){console.error("Failed to persist document record:",i.message)}return a(200,{docId:N,objectKey:n.objectKey,fileName:R,docType:d,contentType:r,uploadedAt:I,extracted:b,autoClassifyResult:P})}function ae(e){return`You are a CPA's document-intake assistant for a Foot Solutions retail franchise in Denton County, Texas.
