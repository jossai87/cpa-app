...
- Initial Franchise Fee Paid This Year: ${o(e.initialFranchiseFeePaidThisYear)} (amortize over ${C.initialFranchiseFeeAmortizationYears} years per Section 197)`:""}
...
Respond with ONLY the JSON object \u2014 no markdown, no commentary.`}async function V(e){let t={messages:[{role:"user",content:[{text:e}]}],inferenceConfig:{maxTokens:4096,temperature:.1}},n=new $.InvokeModelCommand({modelId:W,contentType:"application/json",accept:"application/json",body:JSON.stringify(t)}),r=await Promise.race([U.send(n),new Promise((d,f)=>setTimeout(()=>f(new Error("BEDROCK_TIMEOUT")),29e3))]),b=JSON.parse(new TextDecoder().decode(r.body)).output.message.content[0].text.replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim();return JSON.parse(b)}async function z(e,t){if(!e.body)return a(400,{error:"Request body is required"});let n;try{n=JSON.parse(e.body)}catch{return a(400,{error:"Invalid JSON in request body"})}if(!n.taxYear||n.taxYear<2e3||n.taxYear>2099)return a(400,{error:"taxYear must be between 2000 and 2099",field:"taxYear"});if(n.totalRevenue===void 0||n.totalRevenue<0)return a(400,{error:"totalRevenue is required and must be non-negative",field:"totalRevenue"});if(n.cogs===void 0||n.cogs<0)return a(400,{error:"cogs is required and must be non-negative",field:"cogs"});if(!n.entityType)return a(400,{error:"entityType is required",field:"entityType"});console.log("Tax calculation request:",JSON.stringify(p(n)));let r=v(),i=new Date().toISOString(),x;try{let d=G(n);x=await V(d)}catch(d){let f=d;return f.message==="BEDROCK_TIMEOUT"?(console.error("Bedrock invocation timed out"),a(504,{error:"Tax analysis timed out. Please try again."})):(console.error("Bedrock invocation failed:",f.message),a(502,{error:"AI model returned an unexpected response. Please try again."}))}let b={userId:t,sk:`TAX#${r}`,sessionId:r,taxYear:n.taxYear,entityType:n.entityType,inputData:p(n),useStandards:n.useStandards,bedrockResponse:x,createdAt:i,status:"complete"};try{await y.send(new m.PutCommand({TableName:h,Item:b}))}catch(d){return console.error("DynamoDB write failed:",d.message),a(500,{error:"Failed to save tax session. Please try again."})}return a(200,{sessionId:r,taxYear:n.taxYear,createdAt:i,result:x})}async function X(e){try{let n=((await y.send(new m.QueryCommand({TableName:h,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",ExpressionAttributeValues:{":uid":e,":prefix":"TAX#"},ScanIndexForward:!1,Limit:100,ProjectionExpression:"sessionId, taxYear, entityType, createdAt, #s",ExpressionAttributeNames:{"#s":"status"}}))).Items??[]).map(r=>({sessionId:r.sessionId,taxYear:r.taxYear,entityType:r.entityType,createdAt:r.createdAt,status:r.status}));return a(200,{sessions:n})}catch(t){return console.error("DynamoDB query failed:",t.message),a(500,{error:"Failed to retrieve tax history."})}}async function H(e,t){let n=e.pathParameters?.id;if(!n)return a(400,{error:"Session ID is required"});try{let r=await y.send(new m.GetCommand({TableName:h,Key:{userId:t,sk:`TAX#${n}`}}));if(!r.Item)return a(404,{error:"Session not found"});let i=r.Item;return a(200,{sessionId:i.sessionId,taxYear:i.taxYear,entityType:i.entityType,createdAt:i.createdAt,status:i.status,useStandards:i.useStandards,inputData:i.inputData,result:i.bedrockResponse})}catch(r){return console.error("DynamoDB get failed:",r.message),a(500,{error:"Failed to retrieve tax session."})}}async function Q(e,t){let n=e.pathParameters?.id;if(!n)return a(400,{error:"Session ID is required"});try{return(await y.send(new m.GetCommand({TableName:h,Key:{userId:t,sk:`TAX#${n}`}}))).Item?(await y.send(new m.DeleteCommand({TableName:h,Key:{userId:t,sk:`TAX#${n}`}})),a(200,{sessionId:n,deleted:!0})):a(404,{error:"Session not found"})}catch(r){return console.error("DynamoDB delete failed:",r.message),a(500,{error:"Failed to delete tax session."})}}var K=async e=>{let t=e.requestContext.authorizer.jwt.claims.sub;switch(e.routeKey){case"POST /tax/calculate":return z(e,t);case"GET /tax/history":return X(t);case"GET /tax/history/{id}":return H(e,t);case"DELETE /tax/history/{id}":return Q(e,t);default:return a(404,{error:"Route not found"})}};0&&(module.exports={handler});
