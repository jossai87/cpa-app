"use strict";var z=Object.create;var P=Object.defineProperty;var X=Object.getOwnPropertyDescriptor;var Q=Object.getOwnPropertyNames;var Z=Object.getPrototypeOf,ee=Object.prototype.hasOwnProperty;var te=(e,t)=>{for(var r in t)P(e,r,{get:t[r],enumerable:!0})},j=(e,t,r,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let o of Q(t))!ee.call(e,o)&&o!==r&&P(e,o,{get:()=>t[o],enumerable:!(n=X(t,o))||n.enumerable});return e};var U=(e,t,r)=>(r=e!=null?z(Z(e)):{},j(t||!e||!e.__esModule?P(r,"default",{value:e,enumerable:!0}):r,e)),ne=e=>j(P({},"__esModule",{value:!0}),e);var xe={};te(xe,{handler:()=>he});module.exports=ne(xe);var g=require("@aws-sdk/client-s3"),R=require("@aws-sdk/s3-request-presigner"),A=require("@aws-sdk/client-bedrock-runtime"),W=require("@aws-sdk/client-dynamodb"),y=require("@aws-sdk/lib-dynamodb");var Y=U(require("crypto")),D=new Uint8Array(256),E=D.length;function N(){return E>D.length-16&&(Y.default.randomFillSync(D),E=0),D.slice(E,E+=16)}var u=[];for(let e=0;e<256;++e)u.push((e+256).toString(16).slice(1));function B(e,t=0){return u[e[t+0]]+u[e[t+1]]+u[e[t+2]]+u[e[t+3]]+"-"+u[e[t+4]]+u[e[t+5]]+"-"+u[e[t+6]]+u[e[t+7]]+"-"+u[e[t+8]]+u[e[t+9]]+"-"+u[e[t+10]]+u[e[t+11]]+u[e[t+12]]+u[e[t+13]]+u[e[t+14]]+u[e[t+15]]}var K=U(require("crypto")),L={randomUUID:K.default.randomUUID};function re(e,t,r){if(L.randomUUID&&!t&&!e)return L.randomUUID();e=e||{};let n=e.random||(e.rng||N)();if(n[6]=n[6]&15|64,n[8]=n[8]&63|128,t){r=r||0;for(let o=0;o<16;++o)t[r+o]=n[o];return t}return B(n)}var I=re;var k=new g.S3Client({region:"us-east-1"}),oe=new A.BedrockRuntimeClient({region:"us-east-1"}),ae=new W.DynamoDBClient({region:"us-east-1"}),b=y.DynamoDBDocumentClient.from(ae),S=process.env.DOCS_BUCKET??"",w=process.env.TABLE_NAME??"",se=process.env.BEDROCK_MODEL_ID??"us.amazon.nova-2-lite-v1:0",M=process.env.BEDROCK_PRO_MODEL_ID??"us.amazon.nova-pro-v1:0";function G(e,t){return t||new Set(["lease","line-of-credit","bank-statement","profit-loss","general"]).has(e)?M:se}function a(e,t){return{statusCode:e,headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}}var ie=["application/pdf","text/csv","application/vnd.ms-excel","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/msword","image/png","image/jpeg"];function _(e){switch(e.toLowerCase().split(".").pop()??""){case"pdf":return"application/pdf";case"csv":return"text/csv";case"xlsx":return"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";case"xls":return"application/vnd.ms-excel";case"docx":return"application/vnd.openxmlformats-officedocument.wordprocessingml.document";case"doc":return"application/msword";case"png":return"image/png";case"jpg":case"jpeg":return"image/jpeg";default:return"application/octet-stream"}}var le=["auto","profit-loss","bank-statement","line-of-credit","payroll-summary","royalty-statement","sales-tax-return","fixed-assets","insurance","lease","general"];async function ce(e,t){if(!e.body)return a(400,{error:"Request body is required"});let r;try{r=JSON.parse(e.body)}catch{return a(400,{error:"Invalid JSON in request body"})}if(!r.fileName)return a(400,{error:"fileName is required"});let n=_(r.fileName),o=r.contentType&&r.contentType!==""?r.contentType:n;if(!ie.includes(o))return a(400,{error:"Unsupported file type. Allowed: PDF, CSV, XLSX, PNG, JPEG"});let s=le.includes(r.docType??"")?r.docType:"general",d=`${t}/${s}/${Date.now()}-${I()}-${r.fileName}`;try{let h=new g.PutObjectCommand({Bucket:S,Key:d}),m=await(0,R.getSignedUrl)(k,h,{expiresIn:300});return a(200,{uploadUrl:m,objectKey:d,docType:s,contentType:o,expiresIn:300})}catch(h){return console.error("Failed to create pre-signed URL:",h.message),a(500,{error:"Failed to create upload URL"})}}async function ue(e,t){if(!e.body)return a(400,{error:"Request body is required"});let r;try{r=JSON.parse(e.body)}catch{return a(400,{error:"Invalid JSON in request body"})}if(!r.objectKey)return a(400,{error:"objectKey is required"});if(!r.objectKey.startsWith(`${t}/`))return a(403,{error:"Access denied to this document"});let n,o,s=null;try{let i=await k.send(new g.GetObjectCommand({Bucket:S,Key:r.objectKey}));if(o=i.ContentType??"application/octet-stream",o==="application/octet-stream"||o==="binary/octet-stream"){let l=_(r.objectKey);l!=="application/octet-stream"&&(o=l)}if(n=await i.Body.transformToByteArray(),n.byteLength>8*1024*1024)return a(413,{error:`Document is too large for AI extraction (${Math.round(n.byteLength/1024/1024)}MB). Files must be under 8MB. For large reference documents, store them outside this app.`});(o==="text/csv"||o.startsWith("text/"))&&(s=new TextDecoder().decode(n),s.length>5e4&&(s=s.slice(0,5e4)+`

... [truncated]`))}catch(i){return console.error("Failed to read document from S3:",i.message),a(500,{error:"Failed to read uploaded document"})}function d(i){let l=[],p="uploaded-document";if(o==="application/pdf")l.push({document:{format:"pdf",name:p,source:{bytes:n}}});else if(o==="image/png"||o==="image/jpeg"||o==="image/jpg"){let c=o==="image/png"?"png":"jpeg";l.push({image:{format:c,source:{bytes:n}}})}else o==="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"?l.push({document:{format:"xlsx",name:p,source:{bytes:n}}}):o==="application/vnd.ms-excel"?l.push({document:{format:"xls",name:p,source:{bytes:n}}}):o==="application/vnd.openxmlformats-officedocument.wordprocessingml.document"?l.push({document:{format:"docx",name:p,source:{bytes:n}}}):o==="application/msword"&&l.push({document:{format:"doc",name:p,source:{bytes:n}}});return l.push({text:i}),l}async function h(i,l,p){let c=new A.ConverseCommand({modelId:p,messages:[{role:"user",content:i}],inferenceConfig:{maxTokens:l,temperature:0}}),C=(await Promise.race([oe.send(c),new Promise((be,H)=>setTimeout(()=>H(new Error("BEDROCK_TIMEOUT")),25e3))])).output?.message?.content?.[0]?.text;if(!C)throw new Error("Bedrock returned no text content");return C}let m=r.docType??"auto",T=null,x=null;if(m==="auto")try{let i=de(s),p=(await h(d(i),4096,G("auto",!0))).replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim(),c=JSON.parse(p);T={classifiedAs:c.classifiedAs,confidence:c.classifyConfidence,rationale:c.classifyRationale,...c.bestGuessLabel&&{bestGuessLabel:c.bestGuessLabel}};let F=["profit-loss","bank-statement","line-of-credit","payroll-summary","royalty-statement","sales-tax-return","fixed-assets","insurance","lease"];c.classifyConfidence==="low"||!F.includes(c.classifiedAs)?m="general":m=c.classifiedAs,x=c.extracted??{}}catch(i){let l=i;if(l.message==="BEDROCK_TIMEOUT")return console.error("Bedrock auto-classify timed out"),a(504,{error:"AI processing timed out. This document may be too long or complex. Try a smaller file or pick the document type manually."});console.warn("Auto classify+extract failed, falling back to general:",l.message),m="general",x=null}if(x===null){let i=me(m,s),l=d(i);try{let p=m==="bank-statement"||m==="line-of-credit",c=G(m,!1),C=(await h(l,p?4096:2048,c)).replace(/^```json\s*/i,"").replace(/\s*```$/,"").trim();x=JSON.parse(C)}catch(p){let c=p;return c.message==="BEDROCK_TIMEOUT"?(console.error("Bedrock extraction timed out"),a(504,{error:"AI extraction timed out. This document may be too long. Try a smaller file or split it into pages."})):(console.error("Extraction failed:",c.message),a(502,{error:"Failed to extract data from document"}))}}let O=I(),v=new Date().toISOString(),$=r.fileName||r.objectKey.split("/").pop()?.replace(/^\d+-[0-9a-f-]+-/,"")||"document";if(x===null)return a(502,{error:"Extraction returned no data"});let f=x,V=m==="bank-statement"||m==="line-of-credit",q=f.confidence==="low",J=V?f.categoryTotals??{}:Object.fromEntries(Object.entries(f).filter(([,i])=>typeof i=="number"&&i>0));try{await b.send(new y.PutCommand({TableName:w,Item:{userId:t,sk:`DOC#${v}#${O}`,docId:O,objectKey:r.objectKey,fileName:$,docType:m,contentType:o,uploadedAt:v,appliedTotals:J,flagged:f.flaggedTransactions??[],bankName:f.bankName??null,periodStart:f.periodStart??null,periodEnd:f.periodEnd??null,confidence:f.confidence??null,notes:f.notes??null,autoClassified:T!==null,autoClassifyResult:T??null}}))}catch(i){console.error("Failed to persist document record:",i.message)}return a(200,{docId:O,objectKey:r.objectKey,fileName:$,docType:m,contentType:o,uploadedAt:v,extracted:f,autoClassifyResult:T,isLowConfidence:q})}function de(e){return`You are a CPA's document-intake assistant for a Foot Solutions retail franchise in Denton County, Texas.
...
Respond with ONLY the JSON object.${n}`}async function fe(e){try{let r=((await b.send(new y.QueryCommand({TableName:w,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",ExpressionAttributeValues:{":uid":e,":prefix":"DOC#"},ScanIndexForward:!1,Limit:200}))).Items??[]).map(n=>({docId:n.docId,fileName:n.fileName,docType:n.docType,objectKey:n.objectKey,contentType:n.contentType,uploadedAt:n.uploadedAt,appliedTotals:n.appliedTotals??{},flagged:n.flagged??[],bankName:n.bankName??null,periodStart:n.periodStart??null,periodEnd:n.periodEnd??null,confidence:n.confidence??null,notes:n.notes??null,autoClassified:n.autoClassified??!1,autoClassifyResult:n.autoClassifyResult??null}));return a(200,{documents:r})}catch(t){return console.error("Failed to list documents:",t.message),a(500,{error:"Failed to list documents"})}}async function ye(e,t){let r=e.pathParameters?.id;if(!r)return a(400,{error:"Document id is required"});let n;try{n=(await b.send(new y.QueryCommand({TableName:w,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",FilterExpression:"docId = :docId",ExpressionAttributeValues:{":uid":t,":prefix":"DOC#",":docId":r}}))).Items?.[0]}catch(s){return console.error("Failed to look up document:",s.message),a(500,{error:"Failed to look up document"})}if(!n)return a(404,{error:"Document not found"});let o=n.objectKey;if(!o.startsWith(`${t}/`))return a(403,{error:"Access denied"});try{let s=new g.GetObjectCommand({Bucket:S,Key:o}),d=await(0,R.getSignedUrl)(k,s,{expiresIn:300});return a(200,{downloadUrl:d,fileName:n.fileName,expiresIn:300})}catch(s){return console.error("Failed to create download URL:",s.message),a(500,{error:"Failed to create download URL"})}}async function ge(e,t){let r=e.pathParameters?.id;if(!r)return a(400,{error:"Document id is required"});let n;try{n=(await b.send(new y.QueryCommand({TableName:w,KeyConditionExpression:"userId = :uid AND begins_with(sk, :prefix)",FilterExpression:"docId = :docId",ExpressionAttributeValues:{":uid":t,":prefix":"DOC#",":docId":r}}))).Items?.[0]}catch(d){return console.error("Failed to look up document for delete:",d.message),a(500,{error:"Failed to delete document"})}if(!n)return a(404,{error:"Document not found"});let o=n.sk,s=n.objectKey;if(!s.startsWith(`${t}/`))return a(403,{error:"Access denied"});try{await k.send(new g.DeleteObjectCommand({Bucket:S,Key:s}))}catch(d){console.error("Failed to delete S3 object:",d.message)}try{await b.send(new y.DeleteCommand({TableName:w,Key:{userId:t,sk:o}}))}catch(d){return console.error("Failed to delete metadata record:",d.message),a(500,{error:"Failed to delete document metadata"})}return a(200,{docId:r,appliedTotals:n.appliedTotals??{},deleted:!0})}var he=async e=>{let t=e.requestContext.authorizer.jwt.claims.sub;switch(e.routeKey){case"POST /documents/upload-url":return ce(e,t);case"POST /documents/extract":return ue(e,t);case"POST /documents/bda-job":return a(501,{error:"Not implemented in Phase 1 \u2014 use /documents/extract for CSV"});case"GET /documents":return fe(t);case"GET /documents/{id}/download-url":return ye(e,t);case"DELETE /documents/{id}":return ge(e,t);default:return a(404,{error:"Route not found"})}};0&&(module.exports={handler});
