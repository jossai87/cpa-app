...
async function getSyncStatus() {
    const r = await docClient.send(new lib_dynamodb_1.GetCommand({
        TableName: TABLE_NAME,
...
async function handleImportTaxDefaults(event) {
    ...
    for (const r of rollups) {
        for (const [pt, v] of Object.entries(r.byPaymentType ?? {})) {
            if (!paymentTypeTotals[pt])
                paymentTypeTotals[pt] = { count: 0, amount: 0 };
            paymentTypeTotals[pt].count += v.count;
            paymentTypeTotals[pt].amount += v.amount;
        }
    }
    // Resolve payment-type IDs to friendly labels
    const ptResult = await docClient.send(new lib_dynamodb_1.GetCommand({
        TableName: TABLE_NAME,
    ...
    let topBrands = [];
    const reportingYear = await docClient.send(new lib_dynamodb_1.GetCommand({
        TableName: TABLE_NAME,
    ...
    if (year === currentYear) {
        const invResult = await docClient.send(new lib_dynamodb_1.GetCommand({
            TableName: TABLE_NAME,
        ...
        if (invResult.Item) {
            ...
            if (totalCost > 0)
                endingInventoryCost = Math.round(totalCost * 100) / 100;
        }
    }
    ...
    const summary = {
        ...
        metadata: {
            ...
            avgMarginPct: netSales != null && netMargin != null && netSales > 0
...
async function handleAnalytics(event) {
    ...
    for (const r of rollups) {
        ...
        for (const [typeId, v] of Object.entries(r.byPaymentType)) {
            if (!paymentTotals[typeId])
                paymentTotals[typeId] = { amount: 0, count: 0 };
...
async function handleStaff(event) {
    ...
    const usersResult = await docClient.send(new lib_dynamodb_1.GetCommand({
        TableName: TABLE_NAME,
...
async function handlePurchasing() {
    const [vendorsResult, ordersResult] = await Promise.all([
        docClient.send(new lib_dynamodb_1.GetCommand({ TableName: TABLE_NAME, Key: { userId: OWNER_USER_ID, sk: 'POS#PURCHASING#VENDORS' } })),
        docClient.send(new lib_dynamodb_1.GetCommand({ TableName: TABLE_NAME, Key: { userId: OWNER_USER_ID, sk: 'POS#PURCHASING#ORDERS' } })),
    ]);
...
async function handleReporting(event) {
    const result = await docClient.send(new lib_dynamodb_1.GetCommand({ TableName: TABLE_NAME, Key: { userId: OWNER_USER_ID, sk: 'POS#REPORTING#SALES' } }));
    if (!result.Item) {
        return json(200, {
            rows: [],
            notReady: true,
    ...
    const ytdNetSales = totalsAgg
        ? totalsAgg.reduce((s, r) => s + (r['source_sales.net_sales'] ?? 0), 0)
        : brandSumNetSales;
    const ytdTransactions = totalsAgg
        ? totalsAgg.reduce((s, r) => s + (r['source_sales.transaction_count'] ?? 0), 0)
        : brandSumTxns;
    // Margin: only set if the totals payload actually carries it. Otherwise null.
    let ytdNetMargin = null;
    if (totalsAgg) {
        const sum = totalsAgg.reduce((s, r) => s + (r['source_sales.net_margin'] ?? 0), 0);
    ...
    const repeatRevenue = customerRows
        .filter((r) => (r['source_sales.transaction_count'] ?? 0) > 1)
        .reduce((s, r) => s + (r['source_sales.net_sales'] ?? 0), 0);
    return json(200, {
        fromDate,
        toDate,
        yearStart,
        summary: {
            ...
            avgNetMarginPct: ytdNetMargin !== null && ytdNetSales > 0
        ...
        dailyRows: rows.sort((a, b) => String(a['date.date'] ?? '').localeCompare(String(b['date.date'] ?? ''))),
        monthRows: monthRows.sort((a, b) => String(a['date.month_of_year'] ?? '').localeCompare(String(b['date.month_of_year'] ?? ''))),
        brandRows: brandRows.sort((a, b) => (b['source_sales.net_sales'] ?? 0) - (a['source_sales.net_sales'] ?? 0)),
        returnRows: returnRows.sort((a, b) => (b['source_sales.gross_returns'] ?? 0) - (a['source_sales.gross_returns'] ?? 0)),
        customerInsights: {
            totalCustomers,
            repeatCustomers,
            newCustomers,
...
async function handleInsights(event) {
    ...
    const yearItems = await Promise.all(lookupYears.map((y) => docClient.send(new lib_dynamodb_1.GetCommand({
        TableName: TABLE_NAME,
    ...
    const brandTotals = new Map();
    const returnAccum = new Map();
    const customerAccum = new Map();
    ...
    for (const r of yearItems) {
        if (!r.item || !selectedYears.includes(r.year))
            continue;
        const item = r.item;
        ...
        const brandRows = item['brandRows'] ?? [];
        ...
        const returnRows = item['returnRows'] ?? [];
    ...
    const totalTransactions = aggregateTransactions > 0
