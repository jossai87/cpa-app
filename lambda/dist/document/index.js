...
const NOVA_LITE_MODEL_ID = process.env['BEDROCK_MODEL_ID'] ?? 'us.amazon.nova-2-lite-v1:0';
const NOVA_PRO_MODEL_ID = process.env['BEDROCK_PRO_MODEL_ID'] ?? 'us.amazon.nova-pro-v1:0';
/**
 ...
 */
function pickModel(docType, isAutoClassify) {
...
async function handleUploadUrl(event, userId) {
    if (!event.body)
    ...
    const docType = ALLOWED_DOC_TYPES.includes(body.docType ?? '')
        ? body.docType
        : 'general';
    const objectKey = `${userId}/${docType}/${Date.now()}-${(0, uuid_1.v4)()}-${body.fileName}`;
    try {
        ...
        console.error('Failed to create pre-signed URL:', err.message);
...
async function handleExtract(event, userId) {
    if (!event.body)
        return json(400, { error: 'Request body is required' });
    let body;
    try {
    ...
    try {
        ...
        if (documentBytes.byteLength > 8 * 1024 * 1024) {
            return json(413, {
                error: `Document is too large for AI extraction (${Math.round(documentBytes.byteLength / 1024 / 1024)}MB). ` +
        ...
        console.error('Failed to read document from S3:', err.message);
    ...
    async function callBedrock(content, maxTokens, modelId) {
        ...
        const response = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('BEDROCK_TIMEOUT')), 25000)),
        ]);
    ...
    if (docType === 'auto') {
        try {
            ...
            const rawText = await callBedrock(buildContent(combinedPrompt), 4096, pickModel('auto', true));
            ...
            if (parsed.classifyConfidence === 'low' ||
            ...
            if (error.message === 'BEDROCK_TIMEOUT') {
                ...
                return json(504, {
                    error: 'AI processing timed out. This document may be too long or complex. Try a smaller file or pick the document type manually.',
                });
            }
            console.warn('Auto classify+extract failed, falling back to general:', error.message);
            docType = 'general';
    ...
    if (extracted === null) {
        ...
        try {
            ...
            if (error.message === 'BEDROCK_TIMEOUT') {
                ...
                return json(504, {
                    error: 'AI extraction timed out. This document may be too long. Try a smaller file or split it into pages.',
                });
            }
    ...
    const derivedFileName = body.fileName ||
    ...
    const appliedTotals = isStatement
        ? ext['categoryTotals'] ?? {}
        : Object.fromEntries(Object.entries(ext).filter(([, v]) => typeof v === 'number' && v > 0));
    try {
        ...
        console.error('Failed to persist document record:', err.message);
...
function buildClassifyAndExtractPrompt(csvContent) {
...
function buildExtractionPrompt(docType, csvContent) {
    ...
    const docTypeDescriptions = {
        'profit-loss': 'Extract: totalRevenue, cogs, totalOperatingExpenses, netIncome, rentLeasePayments, utilities, businessInsurancePremiums, marketingAdvertising, professionalFees, totalEmployeeWages.',
        ...
        'royalty-statement': 'Extract: totalRevenue (gross sales reported), royaltyFees, adFundContributions.',
        'sales-tax-return': 'Extract: totalRevenue (taxable sales), salesTaxCollected, salesTaxRemitted.',
        'fixed-assets': 'Extract: totalEquipmentCost (sum of all assets purchased this year), and an array of individual assets with description, cost, placedInServiceDate, depreciationMethod.',
...
function buildBankStatementPrompt(docType, csvContent) {
    const isLOC = docType === 'line-of-credit';
    const csvBlock = csvContent
        ? `\n## Statement Content (CSV/text)\n\`\`\`\n${csvContent}\n\`\`\``
        : '';
    return `You are a CPA's automated bookkeeping assistant analyzing a ${isLOC ? 'business line of credit' : 'business checking'} statement for a Foot Solutions retail franchise in Denton County, Texas.
...
async function handleDownloadUrl(event, userId) {
    const docId = event.pathParameters?.['id'];
    if (!docId)
        return json(400, { error: 'Document id is required' });
    ...
    try {
        ...
        console.error('Failed to look up document:', err.message);
    ...
    try {
        const cmd = new client_s3_1.GetObjectCommand({ Bucket: DOCS_BUCKET, Key: objectKey });
        const downloadUrl = await (0, s3_request_presigner_1.getSignedUrl)(s3Client, cmd, { expiresIn: 300 });
        return json(200, {
            downloadUrl,
            fileName: item['fileName'],
            expiresIn: 300,
        ...
        console.error('Failed to create download URL:', err.message);
        return json(500, { error: 'Failed to create download URL' });
    }
}
async function handleDeleteDocument(event, userId) {
    const docId = event.pathParameters?.['id'];
    if (!docId)
        return json(400, { error: 'Document id is required' });
    ...
    try {
        ...
        console.error('Failed to look up document for delete:', err.message);
    ...
    if (!isSynthetic) {
        try {
            await s3Client.send(new client_s3_1.DeleteObjectCommand({ Bucket: DOCS_BUCKET, Key: objectKey }));
        }
        catch (err) {
    ...
    try {
        ...
        console.error('Failed to delete metadata record:', err.message);
...
async function handleDeleteAllDocuments(userId) {
    let items = [];
    try {
        const result = await docClient.send(new lib_dynamodb_1.QueryCommand({
            TableName: TABLE_NAME,
        ...
        console.error('Failed to list documents for bulk delete:', err.message);
        return json(500, { error: 'Failed to list documents' });
    }
    if (items.length === 0) {
    ...
    const s3Results = await Promise.allSettled(items.map((item) => {
        ...
        return s3Client.send(new client_s3_1.DeleteObjectCommand({ Bucket: DOCS_BUCKET, Key: objectKey }));
    }));
    const s3FailureCount = s3Results.filter((r) => r.status === 'rejected').length;
    if (s3FailureCount > 0) {
        console.warn(`Bulk delete: ${s3FailureCount}/${items.length} S3 deletes failed (continuing with metadata cleanup)`);
    }
    // Delete DynamoDB records in parallel
    const ddbResults = await Promise.allSettled(items.map((item) => docClient.send(new lib_dynamodb_1.DeleteCommand({
        TableName: TABLE_NAME,
        Key: { userId, sk: item['sk'] },
    }))));
    const ddbFailureCount = ddbResults.filter((r) => r.status === 'rejected').length;
    if (ddbFailureCount > 0) {
        console.error(`Bulk delete: ${ddbFailureCount}/${items.length} DynamoDB deletes failed`);
        return json(500, {
...
async function handleResolveFlagged(event, userId) {
    const docId = event.pathParameters?.['id'];
    const indexStr = event.pathParameters?.['index'];
    if (!docId)
        return json(400, { error: 'Document id is required' });
    if (!indexStr)
        return json(400, { error: 'Flagged index is required' });
    ...
    try {
        ...
        console.error('Failed to look up document for resolve:', err.message);
    ...
    const existingResolution = flaggedItem['resolution'];
    const appliedTotals = item['appliedTotals'] ?? {};
    const newAppliedTotals = { ...appliedTotals };
    ...
    if (action === 'unresolve') {
        ...
        if (existingResolution.action === 'apply' &&
        ...
        if (!body.field || !APPLIABLE_FIELDS.has(body.field)) {
            return json(400, {
                error: "field is required and must be one of the supported tax-form fields",
            });
        }
        const amount = typeof body.appliedAmount === 'number' && body.appliedAmount > 0
            ? body.appliedAmount
            : txnAmount;
        if (amount <= 0) {
        ...
        if (existingResolution?.action === 'apply' &&
            ...
            typeof existingResolution.appliedAmount === 'number') {
            const prev = newAppliedTotals[existingResolution.field] ?? 0;
            newAppliedTotals[existingResolution.field] = Math.max(0, prev - existingResolution.appliedAmount);
        }
    ...
    try {
        ...
        console.error('Failed to update flagged resolution:', err.message);
...
async function handleRegisterSupporting(event, userId) {
    if (!event.body)
        return json(400, { error: 'Request body is required' });
    let body;
    try {
    ...
    const docId = (0, uuid_1.v4)();
    const uploadedAt = new Date().toISOString();
    const fileName = body.fileName ?? body.objectKey.split('/').pop() ?? 'supporting-document';
    ...
    try {
        ...
        console.error('Failed to register supporting doc:', err.message);
