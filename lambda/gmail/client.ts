 ...
 *
 * Functions:
 *   searchEmails(query, days, max)   — list message IDs matching a Gmail query
 *   getMessage(id)                   — fetch full headers + plaintext body
 *   summarizeRecentVendorMail(days)  — convenience: scan vendor brand mentions
 */
...
// In-memory access token cache (per Lambda warm invocation)
...
async function getAccessToken(): Promise<string> {
  ...
  const [client, tokens] = await Promise.all([
    ...
    loadSecret<{ refresh_token: string }>(TOKEN_SECRET_NAME),
  ]);
